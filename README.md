...
Each gunslinger can change their look in the Outfitter's **Wardrobe**: skins (colour variants of each alien) and, as the art arrives, add-ons such as gun charms. Items are earned from your record (a free starter in each slot); a locked item shows what earns it, and the results screen says when a round unlocks something. Looks only: nothing you wear changes where you can be hit. The bot wears a random skin each round.

## Settings

Open **Settings** from the top-right of the start screen, or the top-right of the results screen. Changes save on the phone automatically, for the gunslinger you're playing as.
...
- Saved data (never delete these keys; migrate forward): `high-moon-profiles` (gunslingers, versioned, `src/settings/profiles.ts` with `migrate()`), `high-moon-active-profile`, and the pre-v0.7 `duel-settings-v1` / `high-moon-loadout-v1` (kept in step with the active gunslinger so an older build still works), `high-moon-device`, the upload key, IndexedDB `high-moon-logs`. Round logs carry `profile: { id, name }` from v0.7.0. Stats: pure maths in `src/stats/stats.ts` (check: `src/dev/statsCheck.ts`), totals kept in each profile (store version 2) and updated in `finishLog`; `src/stats/backfill.ts` adds pre-v0.7.2 logs once (`backfilled` flag saved with the stats).
- Wardrobe: catalogue and unlock rules in `src/wardrobe/wardrobe.ts` (check: `src/dev/wardrobeCheck.ts`), pictures in `SKIN_ART` / `ITEM_ART` (`src/render/art.ts`), outfit stored in each profile (store version 3). Plan and art review: `docs/V08-WARDROBE.md`.
- Update notice: the build writes `version.json` (plugin in `vite.config.ts`, which also fails the build if `APP_VERSION` and package.json differ); `src/platform/update.ts` checks it on load and every 3 minutes while the start screen is up and shows "Update available" (tap reloads with `?v=` to get past the GitHub Pages cache).
