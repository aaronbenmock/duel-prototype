...
console.log([pad('started (UTC)', 20), pad('phone', 14), pad('player', 12), pad('app', 7), pad('alien/gun', 28), pad('vs', 22), pad('map', 16), pad('result', 10), pad('secs', 6), pad('shots/hits', 11), pad('reloads', 8), pad('you/bot hp', 11), pad('Hz', 4), 'notes'].join(''));
for (const { f, log } of logs) {
  ...
  console.log([
    pad(log.startedAt.slice(0, 19).replace('T', ' '), 20), pad(log.label || log.device, 14), pad(log.profile?.name ?? '-', 12), pad(log.app, 7),
